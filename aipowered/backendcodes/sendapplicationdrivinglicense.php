...
if(isset($_POST['applydrivinglicense'])){
    ...
    $citizen_res = mysqli_query($conn, 
        "SELECT * FROM citizensregistry 
    ...
    $citizen = mysqli_fetch_assoc($citizen_res);

    // 🔎 Check provisional license
    if(empty($citizen['provisional_driving_number'])){
        echo "<script>
        swal('Provisional License Missing',
             'Citizen must have a provisional driving license before applying.',
             'warning');
        </script>";
    }
    ...
    try {
        ...
        $mail->Body = "
            ...
            Provisional License Number: {$citizen['provisional_driving_number']}<br>
